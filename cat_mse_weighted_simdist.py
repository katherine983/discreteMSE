...
def sim_distance(xmi, xixjprob_df, m, r):
    """
    ...
    r : DISTANCE FROM xi THAT A VECTOR, xj, IN xmi WILL BE CONSIDERED A MATCH TO xi
...
def vector_matches(data, m, r, xixjprob_df):
    """
    ...
    matches = sim_distance(xmi, xixjprob_df, m, r) - len(xmi)
    return matches    
    

def sampen(data, m, r, xixjprob_df): 
    """
    ...
    pos = vector_matches(data[:-1], m, r, xixjprob_df) 
    #print(pos)
    
    #get number of matches, store in variable 'mat'
    k = m+1
    mat = vector_matches(data, k, r, xixjprob_df)
    #print(mat)
    if mat == 0:
